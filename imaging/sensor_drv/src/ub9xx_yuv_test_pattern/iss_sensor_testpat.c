...
static int32_t testpat_StreamOn(uint32_t chId, void *pSensorHdl)
{
    ...
    /*Start Streaming*/
    status |= IssSensor_cfgDesScript(ub9xxtestpatDesCSI2Enable, ub9xxInstanceId);
    return (status);
}
...
static int32_t testpat_StreamOff(uint32_t chId, void *pSensorHdl)
{
    ...
    /*Stop Streaming*/
    status |= IssSensor_cfgDesScript(ub9xxtestpatDesCSI2Disable, ub9xxInstanceId);
    return (status);
}
...
static int32_t testpat_PowerOff(uint32_t chId, void *pSensorHdl)
{
    ...
    if(status != 0)
    {
        printf("testpat_PowerOn : powerOffDeserializers returned 0x%x \n", status);
    }
    return status;
}
